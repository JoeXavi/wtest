...
async function fillCheckoutForm(
  ...
  await user.type(screen.getByLabelText(/card number/i), pan);
  await user.type(screen.getByLabelText(/expiry/i), '0827');
  await user.type(screen.getByLabelText(/^cvc$/i), '123');
  await user.type(screen.getByLabelText(/name on card/i), 'Ada Lovelace');
  await user.type(screen.getByLabelText(/^email$/i), 'ada@example.com');
  await user.type(screen.getByLabelText(/full name/i), 'Ada Lovelace');
  await user.type(screen.getByLabelText(/^phone$/i), '+573001112233');
  await user.type(screen.getByLabelText(/id number/i), '1234567890');
  await user.type(screen.getByLabelText(/address line 1/i), 'Calle 100 #10-20');
